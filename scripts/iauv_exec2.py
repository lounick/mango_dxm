    ...
    def rethink_target_cb(self):
        ...
        while self.run_threads_:
            try:
                ...
                # Process item
                if item['new_val']['id'] != self.last_inserted_id_ or item['new_val']['timestamp'] != self.last_timestamp_:
                    if int(item['new_val']['classification']) == 0:
                        pass
                    elif int(item['new_val']['classification']) == 2:
                        try:
                            idx = self.target_ids.index(item['new_val']['id'])
                            self.targets[idx][4] = 2
                        except ValueError:
                    ...
                    elif int(item['new_val']['classification']) == 1:
                        self.targets.append([item['new_val']['lat'], item['new_val']['lon'], item['new_val']['depth'],
                        ...
                        rospy.logwarn("Received unknown classification")
            except r.ReqlTimeoutError:
            ...
            if len(self.my_targets) > 0:
                if not self._action_executing:
                    ...
                    if _action_completed:
                        ...
                        if self.targets[self.target_ids.index(target_id)][4] == 2:
                ...
                if self.reset_targets_count_ > 1200:
                    self.reset_targets_count_ = 0
                    targets = r.table("targets").filter(r.row['classification'] == 1).run(self.connection_)
                    for item in targets:
